...
def build_hud_doc_and_anchors(
    ...
    *,
    ...
    for rally_index, group in enumerate(group_rallies(events, rally_gap_sec), start=1):
        rally_events: list[dict[str, Any]] = []
        for touch_index, event in enumerate(group, start=1):
            time_sec = float(event["time_sec"])
            center = center_for_event(clean_points, raw_points, time_sec, max_track_gap_sec)
            if center is None:
                missing_centers.append({"time_sec": round(time_sec, 6), "event": event})
                continue
            key = event_key(rally_index, "touch", touch_index, time_sec)
            anchors.append(
                {
                    "key": key,
        ...
        rallies.append(
            {
                ...
                "expected_touches": len(rally_events),
                "expected_stalls": 0,
                "events": rally_events,
            }
        )
    ...
    doc = {
        ...
        "annotation_method": "release_touch_classifier_merged_events_with_owlv2_l2_centers",
        "rallies": rallies,
        "events": flat_events,
        "notes": [
            ...
            "No HSV/color fallback anchors are used for touch events.",
        ],
    }
    anchor_doc = {
    ...
    summary = {
        ...
        "rendered_touch_events": len(flat_events),
        "rallies": len(rallies),
...
def write_report(path: Path, manifest: dict[str, Any]) -> None:
    lines = [
        ...
        "## Videos",
        "",
        "| video | split | touches | rallies | anchors | center sources | video/audio/nonblank | output |",
        "| --- | --- | ---: | ---: | ---: | --- | --- | --- |",
    ]
    for row in manifest["videos"]:
        ...
        lines.append(
            f"| `{row['video_id']}` | {row['split']} | {row['rendered_touch_events']} | {row['rallies']} | "
...
def render_release_huds(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for video_id in chosen_ids:
        ...
        events = predicted_touch_events(all_rows, video_id)
        if not events:
