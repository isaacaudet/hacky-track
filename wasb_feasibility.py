...
def main():
    ...
    for i in range(2, len(frames)):
        ...
        for fr in frames[i - 2:i + 1]:
            ...
            chans.append(torch.from_numpy(rgb).permute(2, 0, 1).float() / 255.0)
        ...
        if i % 14 == 2 and len(overlays) < 9:           # save a few overlay tiles
    ...
    if overlays:
        ...
        overlays = [cv2.resize(o, (int(o.shape[1] * hh / o.shape[0]), hh)) for o in overlays]
    ...
    print(f"frames scored (ROI-crop 1024x576 around HSV ball): {len(rows)}")
    print(f"peak confidence: mean={confs.mean():.3f} max={confs.max():.3f} "
          f"min={confs.min():.3f}")
    if len(errs):
        print(f"WASB peak vs HSV ball error (px): median={np.median(errs):.0f} "
              f"p90={np.percentile(errs, 90):.0f}")
        for thr in (40, 80, 150):
            print(f"  WASB peak within {thr}px of ball: {(errs < thr).mean():.2f}")
    print("montage: tmp_proto/wasb_feasibility.png")
