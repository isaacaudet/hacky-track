...
import build_touch_review_montages as montages
...
class TouchReviewMontageTests(unittest.TestCase):
    ...
    def test_renders_silent_motion_montages_without_writing_labels(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            write_json(
                ...
                {
                    "audio_candidates": [
                        {"time_sec": 0.1, "strength": 1.0, "source": "loose_audio_onset"},
                        {"time_sec": 0.3, "strength": 1.0, "source": "loose_audio_onset"},
                    ],
            ...
            manifest = montages.build_montages(self.make_args(root, root / "montages"))

            self.assertEqual(manifest["summary"]["videos_considered"], 1)
            self.assertEqual(manifest["summary"]["montages_rendered"], 2)
            self.assertEqual(manifest["summary"]["candidates_rendered"], 2)
            by_filter = {montage["filter"]: montage for montage in manifest["videos"][0]["montages"]}
            self.assertEqual(by_filter["likely_unchecked"]["times_sec"], [0.2])
            self.assertEqual(by_filter["audio_only"]["times_sec"], [0.3])
            for montage in by_filter.values():
                montage_path = Path(montage["path"])
                self.assertTrue(montage_path.exists())
                cap = cv2.VideoCapture(str(montage_path))
                try:
                    self.assertTrue(cap.isOpened())
                    self.assertGreater(int(cap.get(cv2.CAP_PROP_FRAME_COUNT) or 0), 0)
                finally:
                    cap.release()
